...
public class Menu {
    ...
    public Menu() {
        ...
        ArrayList<String> turnosJugadores = new ArrayList<>(); //ArrayList que guarda el orden de los jugadores (0-primero)...

        accion.crearJugadores(turnosJugadores);//antes de empezar el menu de partida creamos los jugadores
        ...
        while(!salir) {
            ...
            switch(comando){
                ...
                case "comprar":
                    ...
                    break;
                case "hipotecar":
                    if(partes.length!=2) System.out.println("\n Comando incorrecto");
                    else if(this.tablero.getCasillas().get(partes[1])!=null)//si existe la casilla
                    ...
                    break;
                case "pagar":
                    if(partes.length==2 && partes[1].equals("alquiler")) {
                        if (!accionPendiente)//si hay acciones pendientes
                            accionPendiente = accion.pagarAlquiler(jugadorActual, tirada);//se paga el alquiler
                        else
                            System.out.println("No tienes pendiente pagar ningún alquiler");
                    }
                    else System.out.println("\n Comando incorrecto");
                    break;
                case "lanzar":
                        ...
                        if (countTiradas == 0) {//si tienes tiradas pendientes te muestra la tirada
                            ...
                            if (jugadorActual.getAvatar().getEncarcelado() == 0) {//si no esta encarcelado
                                ...
                                if (vecesDobles >= 3) {//si saco dobles 3 veces va a la carcel y se cancelan sus acciones pendientes
                                    ...
                                    accionPendiente = accion.caer(jugadorActual, tirada);
                    ...
                    if(partes.length != 2){
                        ...
                        if(partes[1].equals("turno")) {//si no hay alquileres por pagar o tiradas pendientes deja finalizar el turno
                            if (accionPendiente) {
                                ...
                                System.out.println("Quedan acciones pendientes");
                        }else{
                            System.out.println("Comando incorrecto");
                        }
                    }
